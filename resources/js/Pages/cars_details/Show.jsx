...
const Show = ({ auth, car, suggestedCars, hasVerifiedEmail }) => {
    ...
    const carReviewsWithoutCommentCount = car.reviews.filter(
        (review) => review.comment
    ).length;
    const carReviewsCount = car.reviews.length;
    const companyReviewsCount =
        car.company?.reviews?.filter((review) => review.comment)?.length || 0;
    ...
    const handleCompanyClick = (companyId) => {
        axios
            .post("/companies/session", { company_id: companyId })
            .then((response) => {
    ...
    const loadCarReviews = async (page = 1) => {
        const res = await axios.get(route("cars.reviews.paginated", car.id), {
            params: { page },
        });
                ...
                <div className="container mx-auto px-4 py-8">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="lg:col-span-2">
                            ...
                            (inputType === "xs-range" ||
                                inputType === "xs-s-range") ? (
                                <div className="mt-4 p-3 border rounded-lg">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            <div
                                                className="relative w-12 h-12 rounded-full overflow-hidden border flex-shrink-0 cursor-pointer"
                                                onClick={() =>
                                                    handleCompanyClick(
                                                        car.company.id
                                                    )
                            ...
                            <Tabs defaultValue="description" className="mt-8">
                                <TabsList className="grid grid-cols-3 mb-4">
                                    ...
                                    <TabsTrigger value="reviews">
                                        Reviews ({carReviewsWithoutCommentCount}
                                        )
                                    </TabsTrigger>
                                </TabsList>
                                <TabsContent
                                    value="description"
                                    className="mt-0 xs-s-range:text-xs xs-range:text-xs"
                                >
                                    <Card>
                                    ...
                                    <Card>
                                        <CardContent className="pt-6 xs-s-range:text-xs">
                                            <div className="flex items-center justify-between mb-6">
                                                <div>
                                                    <h3 className="text-lg font-semibold xs-range:text-xs xs-s-range:text-xs">
                                                        Customer Reviews
                                                    </h3>
                                                    <div className="flex items-center mt-1">
                                                        <RatingStars
                                                            rating={car.rates}
                                                        />
                                                        <span className="ml-2 text-sm font-medium xs-range:text-xs xs-s-range:text-xs">
                                                            {car.rates} out of 5
                                                        </span>
                                                        <span className="ml-2 text-sm text-gray-500 xs-range:text-xs xs-s-range:text-xs">
                                                            {carReviewsCount}{" "}
                                                            Review
                                                        </span>
                                                    </div>
                                                ...
                                                <div className="pb-6">
                                                    {reviews.map(
                                                        (review, index) => (
                                                            <div
                                                                key={`${review.id}-${index}`}
                                                                className="flex items-start gap-4 border-b last:border-0 mb-2"
                                                            >
                    ...
                    <div className="mt-16">
                        <div className="relative mb-8">
                            ...
                            <div className="relative flex justify-center">
                                <span className="bg-white px-4 text-lg font-bold text-gray-800 italic">
                                    Suggested
                                </span>
                            </div>
                        </div>
                            ...
                            {suggestedCars.length > 0 ? (
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                                    {suggestedCars.map((car) => (
                                        <Card
                                            key={car.id}
                                            className="overflow-hidden hover:shadow-md transition-shadow car-card"
                                        >
                                                            ...
                                                            {car.tags.map(
                                                                (tag) => (
                                                                    <span
                                                                        ...
                                                                        className="bg-gray-200 px-2 py-1 rounded-full text-xs text-gray-600"
                                                                    >
                                                                        {
                                                                            tag.name
                                                                        }
                        ...
                        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
                            <div className="bg-white rounded-lg w-full max-w-md">
                                ...
                                <UserRatingForm
                                    ...
                                    CompanyName={car.company.company_name}
                                    auth={auth}
                                    reloadReviews={loadCarReviews}
                                />
                            </div>
                        </div>
                    )}
