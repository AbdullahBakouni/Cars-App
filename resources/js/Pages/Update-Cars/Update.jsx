...
export default function Update({ auth, car, hasVerifiedEmail }) {
    ...
    useEffect(() => {
        ...
        const matchedBrand = carData.data.find(
    ...
    const removeImage = (index, imageId) => {
        ...
        if (image.isNew === true) {
            ...
            setData((prevData) => {
                const newImages = prevData.new_images.filter(
    ...
    const handleTagAdd = () => {
        if (
            tagInput.trim() &&
            !data.tags.some((tag) => tag.name === tagInput)
        ) {
            const newTag = {
    ...
    const removeTag = (tagId) => {
        ...
        if (tagToRemove) {
            ...
            setData(
                "tags",
                data.tags.filter((tag) => tag.id !== tagId)
            );
        }
    };
                            ...
                            {imageUrls.length > 0 && (
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mt-4">
                                    {imageUrls.map(
                                        (
                                            image,
                                            index // `image` is now the object with `id` and `url`
                                        ) => (
                                            <div
                                                key={image.id}
