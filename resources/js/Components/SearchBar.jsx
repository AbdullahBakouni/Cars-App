    ...
    const fetchModels = async (brandName) => {
        ...
        try {
            const response = await axios.get(
                `/api/cars/models?brand=${brandName}`
            );
    ...
    const handleKeyPress = (event) => {
        if (event.key === "Enter") {
            if (selectedBrand && selectedModel === null) {
                ...
                Inertia.visit(
                    route("cars.byBodyType", { brand_name: selectedBrand.name })
                );
                                ...
                                <div className="flex items-center">
                                    <div className="w-8 h-8 mr-2 relative">
                                        <LazyLoadImage
                                            ...
                                            width={32}
                                            height={32}
                                            className="object-contain xs-s-range:w-[16] xs-s-range:h-[16px]"
                                ...
                                >
                                    <div className="w-8 h-8 mr-3 relative">
                                        <LazyLoadImage
                                            ...
                                            width={40}
                                            height={40}
                                            className="object-contain xs-s-range:w-[20px] xs-s-range:h-[20px]"
                                            effect="blur" // Optional effect for lazy loading
                                        />
                                    </div>
